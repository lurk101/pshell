...
static void xfunc_die(void) { exit(-1); }

static void* not_const_pp(const void* p) {
    void* pp;
    asm volatile("# forget that p points to const" : /*outputs*/ "=r"(pp) : /*inputs*/ "0"(p));
    return pp;
}
...
static void llist_add_to(llist_t** old_head, void* data);
static void llist_add_to_end(llist_t** list_head, void* data);
...
static void llist_unlink(llist_t** head, llist_t* elm);
static void llist_free(llist_t* elm, void (*freeit)(void* data));
static llist_t* llist_rev(llist_t* list);
static llist_t* llist_find_str(llist_t* first, const char* str);
...
static unsigned strtou(const char* arg, char** endp, int base) {
    unsigned long v;
    char* endptr;
...
static void verror_msg(const char* s, va_list p, const char* strerr) {
#if TODO
    char *msg, *msg1;
    char stack_msg[80];
...
static void simple_error_msg_and_die(const char* s) { error_msg_and_die("%s", s); }

static void die_memory_exhausted(void) { simple_error_msg_and_die(msg_memory_exhausted); }
...
// cmds modifying text[]
static const char modifying_cmds[] = "aAcCdDiIJoOpPrRs"
                                     "xX<>~";
#endif
...
again:
    c = read_key(0, readbuffer, /*timeout:*/ -1);
    if (c == -1) {           // EOF/error
        if (errno == EAGAIN) // paranoia
            goto again;
        go_bottom_and_clear_to_eol();
        cookmode(); // terminal to "cooked"
        simple_error_msg_and_die("can't read user input");
    }
    return c;
}
...
static char* regex_search(char* q, regex_t* preg, const char* Rorig, size_t* len_F, size_t* len_R,
                          char** R) {
...
static void colon(char* buf) {
    ...
    if (cnt == 0)
        return;
    if (strncmp(p, "quit", cnt) == 0 || strcmp(p, "q!") == 0) {
    ...
    if (strncmp(p, "write", cnt) == 0 || strcmp(p, "wq") == 0 || strcmp(p, "wn") == 0 ||
        (p[0] == 'x' && !p[1])) {
        ...
        if (cnt < 0) {
            ...
            modified_count = 0;
            last_modified_count = -1;
            status_line("'%s' %uL, %uC", current_filename, count_lines(text, end - 1), cnt);
            ...
            if (home && *home) {
                ...
                if (stat(exrc, &st) == 0) {
                    if ((st.st_mode & (S_IWGRP | S_IWOTH)) == 0 && st.st_uid == getuid()) {
                        cmds = malloc_open_read_close(exrc, NULL);
                    } else {
