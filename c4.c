...
enum {
    Num = 128,
    Fun, Sys, Glo, Loc, Id, Char, Else, Enum, If, Int, Return, Sizeof,
    While, Assign, Cond, Lor, Lan, Or, Xor, And, Eq, Ne, Lt, Gt, Le,
                ...
                while (le < e) {
                    printf("%8.4s",
                           &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH "
                            ...
                            ",DIV ,MOD ,OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,"[*++le * 5]);
                    if (*le <= ADJ)
                        printf(" %d\n", *++le);
                    else
                        printf("\n");
